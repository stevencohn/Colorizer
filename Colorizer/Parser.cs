...
namespace River.OneMoreAddIn.Colorizer
{
	...
	internal class Parser
	{
		...
		public void Parse(string source, Action<string, string> report)
		{
			...
			if (matches.Count == 0)
			{
				Console.WriteLine($"report(\"{source}\", null); // no-match");
					...
					foreach (var group in groups)
					{
						...
						if (int.TryParse(group.Name, out var scope))
						{
							...
							if (r < language.Rules.Count)
							{
								...
								Console.WriteLine($"newOverride ({newOverride ?? "null"}) from rule {r} /{rule.Pattern}/");
								...
								Console.WriteLine($"scopeOverride ({scopeOverride ?? "null"})");
							}
						}
						else
						{
			...
			{
				Console.WriteLine($"report(\"{source.Substring(index)}\", null); // post");
